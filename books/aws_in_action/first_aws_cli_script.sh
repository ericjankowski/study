...
INSTANCEID=$(aws ec2 run-instances --image-id $AMIID --key-name mykey --instance-type=t2.nano --security-group-ids $SGID --subnet-id $SUBNETID --query "Instances[0].InstanceId" --output text)
...
read -p "Pres [Enter] key to terminate $INSTANCEID ..."
